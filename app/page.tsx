...
const FLOW = [
  {
    title: "Upload",
    text: "A consultant CSV is checked in the browser and sent to the ingest API.",
  },
  {
    title: "Store",
    text: "New people land in MongoDB as “loaded”. Existing ones are never overwritten.",
  },
  {
    title: "Approve",
    text: "Marking someone a decision maker in Compass releases them to outreach.",
  },
  {
    title: "Reach out",
    text: "bench-outreach’s Email Agent sends a role that matches their title.",
  },
  {
    title: "Qualify",
    text: "Replies are researched, followed up and handed to the Bench TA team.",
  },
];
    ...
    <>
      ...
      <section className="relative left-1/2 -mt-6 w-screen -translate-x-1/2 overflow-hidden lg:-mt-8">
        ...
        <div className="relative mx-auto grid min-h-[calc(100dvh-4rem)] max-w-7xl items-center gap-14 px-6 py-16 lg:grid-cols-[1.05fr_1fr] lg:gap-10 lg:px-10">
          <div className="rise flex flex-col items-center text-center lg:items-start lg:text-left">
            ...
            <h1 className="font-display text-[46px] font-semibold leading-[1.02] text-ink sm:text-[58px] xl:text-[64px]">
              Every consultant,
              <br />
              one living{" "}
              <em className="font-medium italic text-accent">pipeline.</em>
            </h1>

            <p className="mt-7 max-w-xl text-[16.5px] leading-relaxed text-muted">
              Upload a roster, watch it land in MongoDB, and follow each
              consultant from the first email to the hand-off — as it happens.
            </p>
            ...
            <ul className="mt-9 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-[13px] text-muted lg:justify-start">
              <li className="flex items-center gap-2">
                ...
                />
                {data
                  ? `${data.total} consultants in MongoDB`
                  : "Reading MongoDB…"}
              </li>
              <li className="flex items-center gap-2">
                <span
                ...
                />
                {data
                  ? `${data.reached.emailed ?? 0} emailed so far`
                  : "Counting outreach…"}
              </li>
              <li className="flex items-center gap-2">
                <span
      ...
      <section className="mx-auto max-w-6xl py-16">
        <p className="eyebrow mb-4">
          <span className="font-display text-[13px] font-medium tracking-normal">
            01
          </span>
        ...
        <h2 className="font-display max-w-2xl text-[34px] font-semibold leading-tight text-ink sm:text-[42px]">
          ...
          <em className="font-medium italic text-accent">conversation.</em>
        </h2>
        ...
        <ol className="mt-10 grid gap-px overflow-hidden rounded-[22px] bg-line shadow-card sm:grid-cols-2 lg:grid-cols-5">
          {FLOW.map((step, index) => (
            <li key={step.title} className="bg-surface p-6">
              <span className="font-display text-[15px] font-medium text-accent">
                0{index + 1}
              </span>
              <p className="font-display mt-3 text-[22px] font-semibold text-ink">
                {step.title}
              </p>
      ...
      <section className="mx-auto max-w-6xl pb-10">
        ...
        <h2 className="font-display max-w-2xl text-[34px] font-semibold leading-tight text-ink sm:text-[42px]">
          Pick a place to{" "}
          <em className="font-medium italic text-accent">start.</em>
        </h2>
          ...
          {explore.map(({ href, label, description, icon: Icon }) => (
            ...
            >
              <div className="flex items-center justify-between">
                <Icon
                  className="h-5 w-5 text-faint transition-colors duration-150 group-hover:text-accent"
                  strokeWidth={1.75}
                />
              ...
              <p className="font-display mt-8 text-[24px] font-semibold text-ink">
                {label}
              </p>
