  ...
  return (
    <ol className="relative">
      {PROGRESS_STAGES.map((step, index) => {
        ...
        const { label, description, color } = STAGE_META[step];
          ...
          <li key={step} className="relative flex gap-4 pb-5 last:pb-0">
            {!isLast && (
              <span
                ...
                style={{ background: reached && index < reachedIndex ? color : 'var(--line-strong)' }}
              />
            )}
            <span
              className={cn(
                'relative z-10 grid h-[27px] w-[27px] shrink-0 place-items-center rounded-full border',
                reached ? 'border-transparent text-white' : 'border-line-strong bg-surface-2 text-faint'
              )}
              style={reached ? { background: color, boxShadow: current ? `0 0 0 4px color-mix(in oklab, ${color} 22%, transparent)` : undefined } : undefined}
            >
